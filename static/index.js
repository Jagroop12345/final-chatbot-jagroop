...
function askBot(){
    ...
    var user_input_text = $('#bot_input_text').val()
    if(user_input_text !=''){
        // console.log('text')
        $('#chat_messages').append('<div class = "user__messages">'+user_input_text+'</div>')
        $('#bot_input_text').val('')
        ...
        $.ajax({
            ...
            contentType: 'application/json',
            success: function (result) {
                $('#chat_messages').append('<div class = "bot__messages">'+result.bot__response+'</div>')
            },
            error: function (result) {
                alert(result.responseJSON.message)
            }
